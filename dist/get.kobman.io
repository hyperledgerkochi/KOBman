...
echo "KOBMAN_ENV_REPO=asa1997/test_repo_a" >> "$kobman_user_config_file"
...
for i in ${env_repos[@]}; do
	...
	[[ ! -f "$remote_list" ]] && echo "Error:No list file found for $repo_name" && ([[ -d "$zip_stage_folder" ]] && rm -rf "$zip_stage_folder") && continue
