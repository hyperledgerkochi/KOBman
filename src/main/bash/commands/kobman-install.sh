...
function __kobman_validate_and_set_version
{
	...
	if [ "$?" -eq "0" ];
	then
        	__kobman_check_and_confirm_existing_version "${namespace}" "${version}"  # should introduce case for mapping kobman->KOBman,tob->TheOrgBook,von->von-network 
        	if [ "$?" -eq "0" ];   							 # check version.txt is empty or not (or version variable is empty or not)
        	then
                	version_value=$1
        		return 0	
		else
                	__kobman_echo_red "Version : ${version} from https://github.com/${namespace_name}/${environment_name} is not available"
			return 1 
		fi
	else
...
function __kobman_check_and_confirm_existing_version
{
      	...
      	git ls-remote --tags "https://github.com/${1}/KOBman" | grep -w 'refs/tags/[0-9]*\.[0-9]*\.[0-9]*' | sort -r | head | grep -o '[^\/]*$' | grep -w "$2" > /dev/null
							# version check is happening only for KOBman
}
